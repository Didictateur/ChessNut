...
function sendRoomUpdate(room) {
  ...
  (room.players || []).forEach((p) => {
    handCounts[p.id] =
      room.hands && room.hands[p.id] ? room.hands[p.id].length : 0;
  });
  ...
  (room.players || []).forEach((p) => {
    ...
    try {
      if (
        base.boardState &&
        base.boardState.pieces &&
        Array.isArray(base.boardState.pieces)
      ) {
          ...
          try {
            ...
            if (piece.invisible) {
              const ownerPlayer = (room.players || []).find(
                (pl) => (pl.color && pl.color[0]) === piece.color,
              );
            ...
            const inv = (room.activeCardEffects || []).find(
              (e) =>
                e &&
                e.type === "invisible" &&
                (e.pieceId === piece.id || e.pieceSquare === piece.square),
            );
        ...
        try {
          ...
          const tous = effects.find(
            (e) =>
              e && e.type === "tous_memes" && e.playerId && e.playerId !== p.id,
          );
          if (
            tous &&
            payload.boardState &&
            Array.isArray(payload.boardState.pieces)
          ) {
            const masked = JSON.parse(JSON.stringify(payload.boardState));
            masked.pieces = masked.pieces.map((pc) => {
              const c = Object.assign({}, pc);
              c.type = "K";
              return c;
            });
    ...
    try {
      payload.visibleSquares = Array.from(
        visibleSquaresForPlayer(room, p.id) || [],
      );
      // attach veiled squares if a brouillard effect targets this recipient
      try {
        const brouillards = (room.activeCardEffects || []).filter(
      ...
      try {
        const mines = (room.activeCardEffects || [])
          .filter((e) => e.type === "mine" && e.playerId === p.id)
          .map((e) => e.square);
    ...
    try {
      ...
      payload.capturedOwn = (room.captured || [])
        .filter((c) => c && c.piece && c.piece.color === shortColor)
        .map((c) => ({
          id: c.id,
          type: c.piece.type,
          originalId: c.piece.id,
          originalSquare: c.piece.square,
...
function maybeDrawAtTurnStart(room, playerId) {
  try {
    ...
    console.error("maybeDrawAtTurnStart error", e);
    sendRoomUpdate(room);
  }
}
...
function endTurnAfterCard(room, senderId) {
  try {
    ...
    try {
      ...
      console.error("updating temporary effects error", e);
    }

    // reset per-turn card-play flags
    try {
      room._cardPlayedThisTurn = {};
    } catch (_) {}
    ...
    console.error("endTurnAfterCard error", e);
  }
}
...
function checkAndHandleVictory(room) {
  try {
    ...
    const hasWhiteKing = pieces.some(
      (p) =>
        p && p.type && String(p.type).toUpperCase() === "K" && p.color === "w",
    );
    const hasBlackKing = pieces.some(
      (p) =>
        p && p.type && String(p.type).toUpperCase() === "K" && p.color === "b",
    );
    ...
    const winner =
      (room.players || []).find(
        (p) => (p.color && p.color[0]) === winnerColor,
      ) || null;
    const loser =
      (room.players || []).find(
        (p) => (p.color && p.color[0]) === loserColor,
      ) || null;
    room.status = "finished";
    try {
      io.to(room.id).emit("game:over", {
    ...
    console.error("checkAndHandleVictory error", e);
    return null;
  }
}
...
function drawCardForPlayer(room, playerId) {
  ...
  if (
    boardVersion !== null &&
    room._lastDrawForPlayer[playerId] === boardVersion
  ) {
    return null;
  }
  ...
  if (room.deck.length === 0) {
    if (!room.noRemise && room.discard && room.discard.length > 0) {
      ...
      for (let i = room.deck.length - 1; i > 0; i--) {
        ...
        const tmp = room.deck[i];
        room.deck[i] = room.deck[j];
        room.deck[j] = tmp;
      }
      (room.players || []).forEach((p) => {
        if (p.socketId)
          io.to(p.socketId).emit("deck:reshuffled", {
            roomId: room.id,
  ...
  try {
    if (room.autoDraw === false) {
      room._playerDrew = room._playerDrew || {};
      room._playerDrew[playerId] = true;
    }
  } catch (_) {}
  sendRoomUpdate(room);
  return card;
...
function computeLegalMoves(room, square) {
  ...
  // coin coin
  try {
    const coin = (room.activeCardEffects || []).find(
      (e) =>
        e &&
        e.type === "coincoin" &&
        ((e.pieceId && e.pieceId === piece.id) || e.pieceSquare === square),
    );
    if (
      coin &&
      Array.isArray(coin.allowedSquares) &&
      coin.allowedSquares.length > 0
    ) {
  ...
  try {
    const hasTeleport = (room.activeCardEffects || []).some(
      (e) =>
        e &&
        e.type === "teleport" &&
        ((e.pieceId && e.pieceId === piece.id) || e.pieceSquare === square),
  ...
  const hasAdoubement = (room.activeCardEffects || []).some(
    (e) =>
      e.type === "adoubement" &&
      ((e.pieceId && e.pieceId === piece.id) || e.pieceSquare === square),
  );
  ...
  const hasFolie = (room.activeCardEffects || []).some(
    (e) =>
      e.type === "folie" &&
      ((e.pieceId && e.pieceId === piece.id) || e.pieceSquare === square),
  );
  ...
  const hasFortification = (room.activeCardEffects || []).some(
    (e) =>
      e.type === "fortification" &&
      ((e.pieceId && e.pieceId === piece.id) || e.pieceSquare === square),
  );

  // detect if the owner of this piece currently has an active 'anneau' effect
  const anneauPlayers = (room.activeCardEffects || [])
    .filter((e) => e.type === "anneau")
    .map((e) => e.playerId);
  const ownerPlayer = (room.players || []).find(
    (p) => (p.color && p.color[0]) === piece.color,
  );
  ...
  if (type === "B" || type === "Q" || type === "R") {
    ...
    const hasRebond = (room.activeCardEffects || []).some(
      (e) =>
        e.type === "rebondir" &&
        ((e.pieceId && e.pieceId === piece.id) || e.pieceSquare === square),
    );
    ...
    directions.forEach(([dx0, dy0]) => {
      if (!hasRebond) {
        ...
        let dx = dx0,
          dy = dy0;
        let cx = x,
          cy = y; // current position while sliding
        let bounced = false;
        while (true) {
          let tx = cx + dx,
            ty = cy + dy;
...
app.get("/rooms/:id", (req, res) => {
  ...
  if (!room)
    return res
      .status(404)
      .json({ error: "room not found", message: "Aucune salle trouvée." });
...
io.on("connection", (socket) => {
  ...
  socket.on("room:join", ({ roomId, playerId }, cb) => {
    ...
    if (!room)
      return (
        cb && cb({ error: "room not found", message: "Aucune salle trouvée." })
      );
    ...
    try {
      if (
        ...
        if (myShort === room.boardState.turn) {
          const hasHand =
            room.hands &&
            room.hands[assignedId] &&
            room.hands[assignedId].length > 0;
          if (!hasHand) maybeDrawAtTurnStart(room, assignedId);
        }
      }
    } catch (e) {
      console.error("post-join draw error", e);
    }
    cb &&
      cb({
        ok: true,
  ...
  socket.on("room:leave", ({ roomId }, cb) => {
    try {
      ...
      if (!room)
        return (
          cb &&
          cb({ error: "room not found", message: "Aucune salle trouvée." })
        );
      if (!playerId)
        return (
          cb &&
          cb({
            error: "not joined",
            message: "Vous n'avez pas rejoint la salle.",
          })
        );
      ...
      return cb && cb({ error: "server_error" });
    }
  });
  ...
  socket.on("room:auto_draw:set", ({ roomId, enabled }, cb) => {
    ...
    if (room.hostId !== sender)
      return (
        cb &&
        cb({
          error: "only the host can change auto-draw",
          message: "Seul l'hôte peut changer le dessin automatique.",
        })
      );
    ...
    try {
      io.to(room.id).emit("room:auto_draw:changed", {
        roomId: room.id,
        enabled: room.autoDraw,
      });
    } catch (_) {}
  ...
  socket.on("room:no_remise:set", ({ roomId, enabled }, cb) => {
    ...
    if (room.hostId !== sender)
      return (
        cb &&
        cb({
          error: "only the host can change no-remise",
          message: "Seul l'hôte peut changer le no-remise.",
        })
      );
    ...
    try {
      io.to(room.id).emit("room:no_remise:changed", {
        roomId: room.id,
        enabled: room.noRemise,
      });
    } catch (_) {}
  ...
  socket.on("game:move", ({ roomId, from, to, promotion }, cb) => {
    ...
    if (room.status === "finished")
      return (
        cb && cb({ error: "game_over", message: "La partie est terminée." })
      );
    try {
      ...
      if (!room.boardState)
        return (
          cb &&
          cb({
            error: "no board state",
            message: "Aucun état de plateau disponible.",
          })
        );

      const board = room.boardState;
      const playerColorShort =
        (roomPlayer.color && roomPlayer.color[0]) || null;
      if (!playerColorShort) return cb && cb({ error: "invalid player color" });

      if (board.turn !== playerColorShort)
        return (
          cb &&
          cb({
            error: "not your turn",
      ...
      if (moving.color !== playerColorShort)
        return (
          cb &&
          cb({
            error: "not your piece",
            message: "Vous ne pouvez pas déplacer une pièce adverse.",
          })
        );

      try {
        const effects = room.activeCardEffects || [];
        const toucher = effects.find(
          (e) => e && e.type === "toucher" && e.playerId === senderId,
        );
        if (toucher && toucher.pieceId && moving.id !== toucher.pieceId) {
          return (
            cb &&
            cb({
              error: "must_move_restricted_piece",
              message: "Vous devez déplacer la pièce restreinte.",
            })
          );
        }
      } catch (_) {}
      ...
      try {
        ...
        const tout = effects2.find(
          (e) => e && e.type === "tout_ou_rien" && e.pieceId === moving.id,
        );
        if (tout) {
          ...
          if (targetIndexCheck === -1) {
            return (
              cb &&
              cb({
                error: "must_capture_to_move",
                message:
                  "Mouvement impossible : cette pièce ne peut se déplacer que pour capturer une pièce adverse.",
              })
            );
          }
        }
      ...
      if (!ok)
        return (
          cb && cb({ error: "illegal move", message: "Mouvement illégal." })
        );
      try {
        const hasBrouillard = (room.activeCardEffects || []).some(
          (e) => e && e.type === "brouillard",
        );
        if (hasBrouillard) {
          ...
          if (!visible.has(to)) {
            return (
              ...
              cb({
                ...
                message: "La destination n'est pas visible.",
              })
            );
          }
        }
      ...
      try {
        const effects = room.activeCardEffects || [];
        const dbl = effects.find(
          (e) => e && e.type === "double" && e.playerId === senderId,
        );
        if (
          dbl &&
          typeof dbl.remainingMoves === "number" &&
          dbl.remainingMoves === 1
        ) {
          ...
          if (occupant) {
            return (
              cb &&
              cb({
                error: "capture_forbidden_double",
                message:
                  "Vous ne pouvez pas capturer lors du deuxième mouvement de 'Jouer deux fois'.",
              })
            );
          }
        }
      ...
      if (targetIndex >= 0) {
        try {
          room.activeCardEffects = room.activeCardEffects || [];
          const sniperIdx = room.activeCardEffects.findIndex(
            (e) =>
              e &&
              e.type === "sniper" &&
              e.pieceId === moving.id &&
          ...
          if (sniperIdx !== -1) {
            ...
            try {
              ...
              const originalOwner = (room.players || []).find(
                (p) => (p.color && p.color[0]) === capturedPiece.color,
              );
              room.captured.push({
                id: uuidv4().slice(0, 8),
                piece: capturedPiece,
                originalOwnerId: (originalOwner && originalOwner.id) || null,
                capturedBy: senderId,
                ts: Date.now(),
              });
              try {
                if (capturedPiece && capturedPiece.invisible)
                  delete capturedPiece.invisible;
              } catch (_) {}
            } catch (_) {
            ...
            try {
              const removedEffect = room.activeCardEffects.splice(
                sniperIdx,
                1,
              )[0];
              try {
                io.to(room.id).emit("card:effect:removed", {
                  roomId: room.id,
                  effectId: removedEffect && removedEffect.id,
                  type: "sniper",
                  playerId: removedEffect && removedEffect.playerId,
                });
              } catch (_) {}
            } catch (_) {}
      ...
      if (capturedPieceForReactions) {
        try {
          room.activeCardEffects = room.activeCardEffects || [];
          const doppel = room.activeCardEffects.find(
            (e) => e && e.type === "doppelganger" && e.pieceId === moving.id,
          );
          if (
            doppel &&
            capturedPieceForReactions &&
            typeof capturedPieceForReactions.type !== "undefined"
          ) {
            ...
            try {
              io.to(room.id).emit("card:effect:applied", {
                roomId: room.id,
                effect: Object.assign({}, doppel, {
                  appliedType: capturedPieceForReactions.type,
                }),
              });
            } catch (_) {}
          }
      ...
      try {
        ...
        for (let i = room.activeCardEffects.length - 1; i >= 0; i--) {
          ...
          if (
            e.type === "rebond" &&
            (e.pieceSquare === from || (e.pieceId && e.pieceId === moving.id))
          ) {
            room.activeCardEffects.splice(i, 1);
            continue;
          }
        ...
        console.error("consuming card effects error", e);
      }
      ...
      try {
        ...
        for (let i = room.activeCardEffects.length - 1; i >= 0; i--) {
          ...
          if (e && e.type === "mine" && e.square === to) {
            ...
            try {
              io.to(roomId).emit("mine:detonated", {
                roomId: room.id,
                ownerId: e.playerId,
                detonatorId: senderId,
                square: to,
                piece: moving,
              });
            } catch (_) {}
            try {
              ...
              if (owner && owner.socketId)
                io.to(owner.socketId).emit("mine:detonated:private", {
                  roomId: room.id,
                  effectId: e.id,
                  square: to,
                  piece: moving,
        ...
        console.error("mine detonation error", err);
      }
      ...
      try {
        ...
        for (let ei = room.activeCardEffects.length - 1; ei >= 0; ei--) {
          ...
          try {
            ...
            for (const pl of players) {
              ...
              if ((ev.playCounts[pl.id] || 0) < threshold) {
                allReached = false;
                break;
              }
            }
        ...
        console.error("brouillard playcount update error", e);
      }
      ...
      try {
        if (room && room._freeMoveFor && room._freeMoveFor === senderId) {
          ...
          try {
            delete room._freeMoveFor;
          } catch (_) {
            room._freeMoveFor = null;
          }
          try {
            io.to(room.id).emit("card:free_move_consumed", {
              roomId: room.id,
              playerId: senderId,
            });
          } catch (_) {}
      ...
      try {
        ...
        for (let i = room.activeCardEffects.length - 1; i >= 0; i--) {
          ...
          if (e.type === "double" && e.playerId === senderId) {
            const newRemaining =
              typeof e.remainingMoves === "number"
                ? e.remainingMoves - 1
                : (e.remainingMoves || 2) - 1;
            if (newRemaining > 0) {
        ...
        console.error("double move consume error", err);
      }
  ...
  socket.on("game:start", ({ roomId }, cb) => {
    ...
    if (!room)
      return (
        ...
        cb({
          ...
          message: "La partie n'a pas été trouvée.",
        })
      );
    ...
    if (!senderId)
      return (
        ...
        cb({
          error: "not joined",
          message: "Vous n'avez pas rejoint la partie.",
        })
      );
    if (room.players.length < 2)
      return (
        ...
        cb({
          ...
          message: "Il faut 2 joueurs pour commencer.",
        })
      );
    if (!room.hostId || room.hostId !== senderId)
      return (
        ...
        cb({
          ...
          message: "Seul l'hôte peut commencer la partie.",
        })
      );
    ...
    try {
      if (room.boardState && room.boardState.turn) {
        ...
        const firstPlayer = room.players.find(
          (p) => (p.color && p.color[0]) === firstColor,
        );
        if (firstPlayer) {
          const hasHand =
            room.hands &&
            room.hands[firstPlayer.id] &&
            room.hands[firstPlayer.id].length > 0;
  ...
  socket.on("room:deck:set", ({ roomId, selected }, cb) => {
    ...
    if (room.hostId !== sender)
      return (
        cb &&
        cb({
          error: "only host can set deck",
          message: "Seul l'hôte peut définir la pioche.",
        })
      );
    try {
      ...
      const pool =
        room.deck && Array.isArray(room.deck) && room.deck.length
          ? room.deck.concat(master)
          : master;
      const byId = {};
      pool.forEach((c) => {
        if (c && c.id) byId[c.id] = c;
        if (c && c.cardId) byId[c.cardId] = c;
      });
      const newDeck = [];
      sel.forEach((sid) => {
        const c = byId[sid];
        if (c) newDeck.push(Object.assign({}, c));
      });
      if (newDeck.length === 0) {
      ...
      console.error("room:deck:set error", err);
      return cb && cb({ error: "server_error" });
    }
  });
    ...
    try {
      ...
      return (
        ...
        cb({
          ...
          moves: [],
          message: "La case sélectionnée est invalide.",
        })
      );
    }
  });
  ...
  socket.on("game:select", ({ roomId, square }, cb) => {
    ...
    try {
      socket.data.lastSelectedSquare = square || null;
    } catch (e) {}
    let moves = [];
    try {
      if (square) {
        const effects = room.activeCardEffects || [];
        const isAffectedByTous = effects.some(
          (e) =>
            e && e.type === "pareil" && e.playerId && e.playerId !== playerId,
        );
        if (isAffectedByTous) {
          const tempRoom = Object.assign({}, room);
          tempRoom.boardState = JSON.parse(
            JSON.stringify(room.boardState || {}),
          );
          if (Array.isArray(tempRoom.boardState.pieces)) {
            tempRoom.boardState.pieces = tempRoom.boardState.pieces.map(
              (pc) => {
                const c = Object.assign({}, pc);
                c.type = "K";
                return c;
    ...
    try {
      const effects = room.activeCardEffects || [];
      const toucherEffects = effects.filter(
        (e) => e && e.type === "toucher" && e.playerId === playerId,
      );
      if (toucherEffects && toucherEffects.length > 0) {
        const selectedPiece = (
          (room.boardState && room.boardState.pieces) ||
          []
        ).find((p) => p.square === square);
        const allowedPieceIds = toucherEffects
          .map((e) => e.pieceId)
          .filter(Boolean);
        if (
          !selectedPiece ||
          allowedPieceIds.indexOf(selectedPiece.id) === -1
        ) {
          moves = [];
        }
      }
  ...
  socket.on("player:draw", ({ roomId }, cb) => {
    ...
    if (!board)
      return (
        ...
        cb({
          error: "no board state",
          message: "L'état du plateau est introuvable.",
        })
      );
    const roomPlayer = room.players.find((p) => p.id === senderId);
    if (!roomPlayer)
    ...
    if (room.autoDraw)
      return (
        cb &&
        cb({
          error: "auto_draw_enabled",
          message: "Le tirage manuel n'est pas autorisé.",
        })
      );
    room._cardPlayedThisTurn = room._cardPlayedThisTurn || {};
    if (room._cardPlayedThisTurn[senderId])
      return (
        cb &&
        cb({
          error: "card_already_played_this_turn",
    ...
    try {
      ...
      try {
        if (room._playerDrewPrev && room._playerDrewPrev[senderId]) {
          try {
            console.log(
              "player:draw: rejected because player drew on previous turn (prev marker true) for",
              senderId,
            );
          } catch (_) {}
          return (
            cb &&
            cb({
              error: "drew_last_turn",
              message:
                "Vous avez pioché lors de votre précédent tour et ne pouvez pas piocher maintenant.",
            })
          );
        }
      } catch (_) {}
      ...
      if (!drawn) {
        return (
          cb &&
          cb({ error: "no_card_drawn", message: "Aucune carte n'a été tirée." })
        );
      }
      board.version = (board.version || 0) + 1;
      room._cardPlayedThisTurn = room._cardPlayedThisTurn || {};
      ...
      try {
        ...
        console.error("decrement-after-draw error", e);
      }
      ...
      try {
        io.to(room.id).emit("player:drew", {
          roomId: room.id,
          playerId: senderId,
        });
      } catch (_) {}
      ...
      console.error("player:draw error", err);
      return cb && cb({ error: "server_error" });
    }
  });
    ...
    const available = [
      {
        id: "invert",
        name: "Invert Turn",
        description: "Swap movement directions for one move",
      },
      {
        id: "teleport",
        name: "Teleport",
        description: "Move one piece to any empty square",
      },
    ];
  ...
  socket.on("card:play", ({ roomId, playerId, cardId, payload }, cb) => {
    ...
    try {
      ...
      console.error("card play pre-check error", e);
    }
    const played = {
      id: uuidv4().slice(0, 8),
      playerId: senderId,
    ...
    try {
      const isRebond =
        typeof cardId === "string" && cardId.indexOf("rebond") !== -1;
      const isAdoub =
        typeof cardId === "string" && cardId.indexOf("adoubement") !== -1;
      const isFolie =
        ...
        if (
          ...
          return (
            ...
            cb({
              ...
              message: "Aucune cible valide n'a été sélectionnée.",
            })
          );
        }
        payload = payload || {};
    ...
    try {
      ...
      const idx = hand.findIndex(
        (c) =>
          (c.id && c.id === (payload && payload.id)) ||
          (c.cardId && c.cardId === cardId) ||
          (c.id && c.id === cardId),
      );
      if (idx === -1) {
        return (
          ...
          cb({
            ...
            message: "Vous ne possedez pas cette carte.",
          })
        );
      }
    ...
    try {
      ...
      if (totems && totems.length) {
        const blocking = totems.find(
          (t) => t && t.playerId && t.playerId !== senderId,
        );
        if (blocking) {
          try {
            room.activeCardEffects = (room.activeCardEffects || []).filter(
              (e) => !(e && e.id === blocking.id),
            );
          } catch (_) {}
          try {
            io.to(room.id).emit("card:effect:removed", {
              roomId: room.id,
              effectId: blocking.id,
              type: "totem",
              playerId: blocking.playerId,
            });
          } catch (_) {}
          try {
            io.to(room.id).emit("card:play_blocked", {
              roomId: room.id,
              played,
              blockedBy: "totem",
              protectedPlayerId: blocking.playerId,
            });
          } catch (_) {}
          try {
            socket.emit("card:play_blocked:private", {
              ok: true,
              blocked: true,
              reason: "totem",
              protectedPlayerId: blocking.playerId,
              message: "Votre carte a été annulée par un totem d'immunité.",
            });
          } catch (_) {}
          try {
            const protectedPlayer = (room.players || []).find(
              (p) => p && p.id === blocking.playerId,
            );
            if (protectedPlayer && protectedPlayer.socketId) {
              io.to(protectedPlayer.socketId).emit("notification", {
                type: "totem_consumed",
                roomId: room.id,
                message:
                  "Votre totem d'immunité a annulé la dernière carte jouée par l'adversaire.",
              });
            }
          } catch (_) {}
          try {
      ...
      console.error("totem immunity check error", e);
    }
    ...
    try {
      ...
      if (cardId === "adoubement") {
        try {
          ...
          console.error("adoubement effect error", e);
        }
      }
      ...
      // doppelganger
        ...
        try {
          ...
          try {
            ...
            console.error("apply persistent doppelganger error", e);
            played.payload = Object.assign({}, payload, {
              applied: "doppelganger",
              appliedTo: target,
            });
          ...
          console.error("doppelganger minimal apply error", e);
        }
      } else if (
        cardId === "folie" ||
        (typeof cardId === "string" && cardId.indexOf("folie") !== -1)
        ...
        try {
          ...
          console.error("folie effect error", e);
        }
      }
      ...
      // fortification
        ...
        try {
          ...
          played.payload = Object.assign({}, payload, {
            applied: "fortification",
            appliedTo: target,
          });
        } catch (e) {
          console.error("fortification effect error", e);
        }
      }
      ...
      // toucher
        ...
        try {
          ...
          const targetOwner =
            (room.players || []).find(
              (p) => (p.color && p.color[0]) === targetPiece.color,
            ) || null;
          room.activeCardEffects = room.activeCardEffects || [];
          const effect = {
            id: played.id,
            type: "toucher",
            playerId: (targetOwner && targetOwner.id) || null,
            pieceId: targetPiece.id,
      ...
      // parrure
        ...
        try {
          ...
          if (
            ...
            targetPiece.color === playerColorShort ||
            !targetPiece.type ||
            targetPiece.type.toLowerCase() !== "q"
          ) {
            return (
              cb &&
              cb({
          ...
          try {
            ...
            console.error("parrure apply error", e);
          }
        } catch (e) {
          console.error("parrure effect error", e);
        }
      }
      ...
      // sniper
        ...
        try {
          const board = room.boardState;
          let source =
            (payload && payload.sourceSquare) ||
            (payload && payload.targetSquare) ||
            null;
          if (!source) {
            try {
              source = socket.data && socket.data.lastSelectedSquare;
            } catch (e) {
              source = null;
            }
          }
          ...
          const playerColorShort =
            (roomPlayer && roomPlayer.color && roomPlayer.color[0]) || null;
          const srcPiece = ((board && board.pieces) || []).find(
            (p) => p.square === source,
          );
          if (
            !board ||
            !source ||
            !srcPiece ||
            srcPiece.color !== playerColorShort
          ) {
            try {
              room.hands = room.hands || {};
              room.hands[senderId] = room.hands[senderId] || [];
            ...
            return (
              cb &&
              cb({
                error: "no valid source selected",
                message: "Aucune source valide n'a été sélectionnée.",
              })
            );
          }
          room.activeCardEffects = room.activeCardEffects || [];
          ...
          console.error("sniper binding error", e);
        }
      }
        ...
        try {
          ...
          if (targetPiece.type && targetPiece.type.toLowerCase() === "k") {
            return (
              cb &&
              cb({
                error: "cannot_target_king",
                message: "Vous ne pouvez pas cibler un roi.",
              })
            );
          }
          const targetOwner =
          ...
          console.error("tout_ou_rien effect error", e);
        }
      }
      ...
      // inversion
        ...
        try {
          ...
          const tgtPiece = ((board && board.pieces) || []).find(
            (p) => p.square === target,
          );
          if (
            !board ||
            ...
            !tgtPiece ||
            srcPiece.color !== playerColorShort ||
            tgtPiece.color === playerColorShort
          ) {
            try {
              room.hands = room.hands || {};
              room.hands[senderId] = room.hands[senderId] || [];
          ...
          try {
            ...
            try {
              ...
              room.activeCardEffects.forEach((e) => {
                try {
                  if (e && e.pieceSquare && e.pieceSquare === sSquare)
                    e.pieceSquare = tSquare;
                  else if (e && e.pieceSquare && e.pieceSquare === tSquare)
                    e.pieceSquare = sSquare;
                } catch (_) {}
              });
            } catch (_) {}
            try {
            ...
            try {
              io.to(room.id).emit("card:effect:applied", {
                ...
                effect: {
                  id: played.id,
                  type: "inversion",
                  swapped: [srcPiece.id, tgtPiece.id],
                  playerId: senderId,
                },
              });
            } catch (_) {}
          } catch (e) {
            console.error("inversion apply error", e);
          }
        } catch (e) {
          console.error("inversion effect error", e);
        }
      }
      ...
      // teleportation
        ...
        try {
          ...
          console.error("teleport effect error", e);
        }
      }
      ...
      // promotion
        ...
        try {
          ...
          if (
            ...
            !targetPiece ||
            targetPiece.color !== playerColorShort ||
            (targetPiece.type && String(targetPiece.type).toLowerCase() !== "p")
          ) {
            played.payload = Object.assign({}, payload, {
              applied: "promotion_failed",
              attemptedTo: target,
            });
            try {
            ...
            played.payload = Object.assign({}, payload, {
              ...
              appliedTo: target,
              fromType: oldType,
              toType: targetPiece.type,
            });
          }
        } catch (e) {
          console.error("promotion effect error", e);
        }
      }
      ...
      // kamikaze
        ...
        try {
          ...
          function neighbors(sq) {
            ...
            for (let dx = -1; dx <= 1; dx++)
              for (let dy = -1; dy <= 1; dy++) {
                ...
                out.push(
                  String.fromCharCode("a".charCodeAt(0) + nx) + (ny + 1),
                );
              }
            return out;
          }
          ...
          for (let i = (board.pieces || []).length - 1; i >= 0; i--) {
            ...
            if (p && affected.indexOf(p.square) !== -1) {
              ...
              try {
                room.captured = room.captured || [];
                const originalOwner = (room.players || []).find(
                  (pl) => (pl.color && pl.color[0]) === cp.color,
                );
                room.captured.push({
                  id: uuidv4().slice(0, 8),
                  piece: cp,
          ...
          try {
            io.to(room.id).emit("mine:detonated", {
              roomId: room.id,
              square: target,
            });
          } catch (_) {}
          played.payload = Object.assign({}, payload, {
            ...
            appliedTo: target,
            affected: affected,
            removedCount: removedPieces.length,
          });
          try {
            const end = checkAndHandleVictory(room);
            if (end && end.over) {
          ...
          console.error("kamikaz effect error", e);
        }
      }
      ...
      // coin coin
        ...
        try {
          ...
          const playerColorShort =
            (roomPlayer && roomPlayer.color && roomPlayer.color[0]) || null;
          const piece = ((board && board.pieces) || []).find(
            (p) => p.square === source,
          );
          ...
          if (
            ...
            !source ||
            !piece ||
            piece.color !== playerColorShort ||
            corners.indexOf(source) === -1
          ) {
            try {
              room.hands = room.hands || {};
              room.hands[senderId] = room.hands[senderId] || [];
            ...
            return (
              cb &&
              cb({
                error: "no valid corner piece selected",
                message: "Aucun coin valide n'a été sélectionné.",
              })
            );
          }
          const emptyCorners = corners.filter((c) => {
            return !(board.pieces || []).some((p) => p.square === c);
          });
          ...
          if (!destChoices || destChoices.length === 0) {
            ...
            return (
              cb &&
              cb({
                error: "no empty destination corner available",
                message: "Aucun coin de destination vide n'est disponible.",
              })
            );
          }
          const effect = {
            ...
            pieceId: piece.id,
            pieceSquare: source,
            allowedSquares: destChoices.slice(0),
            remainingTurns: 1,
            ts: Date.now(),
          };
          room.activeCardEffects = room.activeCardEffects || [];
          ...
          played.payload = Object.assign({}, payload, {
            applied: "coincoin",
            from: source,
            allowed: destChoices.slice(0),
          });
        } catch (e) {
          console.error("coincoin effect error", e);
        }
      }
        ...
        try {
          ...
          if (
            ...
            return (
              cb &&
              cb({
                error: "no pieces to shuffle",
                message: "Aucune pièce à mélanger.",
              })
            );
          }
          const pieces = board.pieces;
          ...
          for (let yy = 0; yy < h; yy++) {
            for (let xx = 0; xx < w; xx++) {
              allSquares.push(
                String.fromCharCode("a".charCodeAt(0) + xx) + (yy + 1),
              );
            }
          }
          if (allSquares.length < pieces.length) {
            played.payload = Object.assign({}, payload, {
              applied: "melange_failed",
            ...
            pieces.forEach((p) => {
              try {
                if (p && p.color === "b" && p.square) {
                  occupiedByBlack[p.square] = true;
                }
              } catch (_) {}
            });
            ...
            pieces.forEach((p) => {
              try {
                if (p && p.color === "w" && p.square) {
                  occupiedByWhite[p.square] = true;
                }
              } catch (_) {}
            });
            const blackSquares = Object.keys(occupiedByBlack);
            for (let i = blackSquares.length - 1; i > 0; i--) {
              const j = Math.floor(Math.random() * (i + 1));
              const tmp = blackSquares[i];
              blackSquares[i] = blackSquares[j];
              blackSquares[j] = tmp;
            }
            ...
            pieces.forEach((p) => {
              try {
                if (p && p.color === "b") {
                  newBlackSquareByPieceId[p.id] =
                    blackSquares[blackIndex] || p.square;
                  blackIndex++;
                }
              } catch (_) {}
            });
            const whiteSquares = Object.keys(occupiedByWhite);
            for (let i = whiteSquares.length - 1; i > 0; i--) {
              const j = Math.floor(Math.random() * (i + 1));
              const tmp = whiteSquares[i];
              whiteSquares[i] = whiteSquares[j];
              whiteSquares[j] = tmp;
            }
            ...
            pieces.forEach((p) => {
              try {
                if (p && p.color === "w") {
                  newWhiteSquareByPieceId[p.id] =
                    whiteSquares[whiteIndex] || p.square;
                  whiteIndex++;
                }
              } catch (_) {}
            });
        ...
        try {
          ...
          if (
            ...
            return (
              cb &&
              cb({
                error: "no pieces to transform",
                message: "Aucune pièce à transformer.",
              })
            );
          }
          const pieces = board.pieces;
          ...
          for (let i = 0; i < pieces.length; i++) {
            ...
            if (t === "P") {
              const choice =
                transformChoices[
                  Math.floor(Math.random() * transformChoices.length)
                ];
              p.type = choice;
              if (p.promoted)
                try {
          ...
          try {
            io.to(room.id).emit("card:effect:applied", {
              roomId: room.id,
              effect: { id: played.id, type: "revolution", mapping },
            });
          } catch (_) {}
          played.payload = Object.assign({}, payload, {
            applied: "revolution",
          ...
          console.error("revolution effect error", e);
        }
      }
      ...
      // invisible
        ...
        try {
          ...
          console.error("invisible effect error", e);
        }
      }
        ...
        try {
          ...
          if (!board || !targetPlayer || targetPlayer.id === senderId) {
            ...
            return (
              cb &&
              cb({
                error: "no valid target player",
                message: "Aucun joueur cible valide n'a été sélectionné.",
              })
            );
          }
          ...
          played.payload = Object.assign({}, payload, {
            applied: "brouillard",
            appliedToPlayer: targetPlayer.id,
          });
        } catch (e) {
          console.error("brouillard effect error", e);
        }
      }
      ...
      // anneau
        ...
        try {
          ...
          const effect = {
            ...
            playerId: senderId,
            ts: Date.now(),
            remainingTurns: (payload && payload.turns) || 1,
          };
          room.activeCardEffects.push(effect);
          try {
            io.to(room.id).emit("card:effect:applied", {
          ...
          console.error("anneau effect error", e);
        }
      }
      ...
      // totem
        ...
        try {
          ...
          console.error("totem effect error", e);
        }
      }
      ...
      //mine
        ...
        try {
          ...
          const targetOccupied = ((board && board.pieces) || []).find(
            (p) => p.square === target,
          );
          ...
          console.error("mine placement error", e);
        }
      }
      ...
      // jouer deux fois
        ...
        try {
          ...
          const effect = {
            ...
            playerId: senderId,
            ts: Date.now(),
            remainingMoves: (payload && payload.moves) || 2,
          };
          room.activeCardEffects.push(effect);
          try {
            io.to(room.id).emit("card:effect:applied", {
          ...
          console.error("double effect error", e);
        }
      }
      ...
      // tous les même
        ...
        try {
          ...
          const effect = {
            ...
            playerId: senderId,
            ts: Date.now(),
            remainingTurns: (payload && payload.turns) || 2,
            decrementOn: "opponent",
          };
          room.activeCardEffects.push(effect);
          try {
            io.to(room.id).emit("card:effect:applied", {
          ...
          console.error("tous_memes effect error", e);
        }
      }
        ...
        try {
          ...
          if (
            ...
            const effect = {
              ...
              fromColor: oldColor,
              toPlayerId: senderId,
              originalOwnerId: originalOwner && originalOwner.id,
              playerId: senderId,
              ts: Date.now(),
            };
            room.activeCardEffects.push(effect);
          ...
          console.error("steal effect error", e);
        }
      }
      ...
      // vole d'une carte
        ...
        try {
          ...
          if (!targetPlayer || targetPlayer.id === senderId) {
            played.payload = Object.assign({}, payload, {
              applied: "steal_card_failed",
              attemptedTo: targetPlayerId,
            });
            try {
              const owner = (room.players || []).find((p) => p.id === senderId);
              if (owner && owner.socketId)
            ...
            if (!victimHand || victimHand.length === 0) {
              played.payload = Object.assign({}, payload, {
                applied: "steal_card_failed_empty",
                attemptedTo: targetPlayerId,
              });
              try {
                const owner = (room.players || []).find(
              ...
              played.payload = Object.assign({}, payload, {
                ...
                stolenCardId: stolen.cardId || stolen.id,
              });
              try {
                const stealer = (room.players || []).find(
                  (p) => p.id === senderId,
                );
                if (stealer && stealer.socketId)
                  io.to(stealer.socketId).emit("card:stolen", {
                    roomId: room.id,
                    from: targetPlayerId,
                    card: stolen,
                  });
              ...
              try {
                ...
                if (victim && victim.socketId)
                  io.to(victim.socketId).emit("card:lost", {
                    roomId: room.id,
                    lostCount: 1,
                  });
              } catch (_) {}
          ...
          console.error("steal-card effect error", e);
        }
      }
      ...
      // carte sans effet
        ...
        try {
          ...
          console.error("no_effect card error", e);
        }
      }
      ...
      // resurrection
        ...
        try {
          ...
          const playerColorShort =
            (roomPlayer && roomPlayer.color && roomPlayer.color[0]) || null;
          const available = (room.captured || []).filter(
            (c) => c && c.piece && c.piece.color === playerColorShort,
          );
          if (!available || available.length === 0) {
            played.payload = Object.assign({}, payload, {
              applied: "resurrection_failed_no_captured",
            });
            try {
              const owner = (room.players || []).find((p) => p.id === senderId);
              if (owner && owner.socketId)
                io.to(owner.socketId).emit("card:effect:applied", {
                  ...
                  effect: {
                    id: played.id,
                    type: "resurrection_failed_no_captured",
                    playerId: senderId,
                    ts: Date.now(),
                  },
                });
            ...
            const selectedId =
              ...
              (payload.captureId ||
                payload.capturedId ||
                payload.targetCapturedId ||
                payload.selectedCapturedId);
            let capturedEntry = null;
            if (selectedId) {
              const idx = (room.captured || []).findIndex(
                (c) =>
                  c &&
                  c.id === selectedId &&
                  c.piece &&
                  c.piece.color === playerColorShort,
              );
            ...
            if (!capturedEntry) {
              played.payload = Object.assign({}, payload, {
                applied: "resurrection_failed_no_valid",
              });
              try {
                const owner = (room.players || []).find(
                  (p) => p.id === senderId,
                ...
                if (owner && owner.socketId)
                  io.to(owner.socketId).emit("card:effect:applied", {
                    ...
                    effect: {
                      id: played.id,
                      type: "resurrection_failed_no_valid",
                      playerId: senderId,
                      ts: Date.now(),
                    },
                  });
              ...
              const board = room.boardState;
              const occupied = ((board && board.pieces) || []).find(
                (p) => p.square === target,
              );
              if (!board || !target || occupied) {
                played.payload = Object.assign({}, payload, {
                  applied: "resurrection_failed_bad_square",
                  attemptedTo: target,
                });
                try {
                  const owner = (room.players || []).find(
                  ...
                  if (owner && owner.socketId)
                    io.to(owner.socketId).emit("card:effect:applied", {
                      ...
                      effect: {
                        id: played.id,
                        type: "resurrection_failed_bad_square",
                        playerId: senderId,
                        square: target,
                        ts: Date.now(),
                      },
                ...
                for (let i = room.captured.length - 1; i >= 0; i--) {
                  if (
                    room.captured[i] &&
                    room.captured[i].id === capturedEntry.id
                  ) {
                    room.captured.splice(i, 1);
                    break;
                  }
                }
                ...
                played.payload = Object.assign({}, payload, {
                  ...
                  appliedTo: target,
                  resurrectedId: newPiece.id,
                  resurrectedType: newPiece.type,
                });
              }
            }
          }
        } catch (e) {
          console.error("resurrection effect error", e);
        }
      }
    ...
    try {
      ...
      if (countsAsMove) {
        try {
          ...
          if (board) {
            ...
            try {
              ...
              console.error(
                "updating temporary effects error (inversion)",
                err,
              );
            }
          ...
          console.error("inversion end-of-turn handling error", err);
          sendRoomUpdate(room);
        }
      } else {
        ...
        sendRoomUpdate(room);
        try {
          io.to(room.id).emit("card:free_move_allowed", {
            roomId: room.id,
  ...
  try {
    for (const [id, room] of rooms.entries()) {
      try {
        ...
        console.error("periodic room check error for", id, e);
      }
    }
  } catch (e) {
    console.error("periodic victory sweep error", e);
  }
}, 1000);
